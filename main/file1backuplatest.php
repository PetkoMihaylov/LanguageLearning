...
function check_word($answerWords, $inputWords, $answer_length, $input_length, $sim, $typo, $j)
{
		...
		else if(count($answerWords) > 0)
		{
			if(count($answerWords) == count($inputWords))
			{
				for($j = 0; $j < count($answerWords), $j < count($inputWords); $j++)
				{
					...
					{
						check_word($answerWords, $inputWords, $answer_length, $input_length, $sim, $typo, $j);
						/* if($answer_length == $phrase_length)
						{
							if($sim < 4)
							{
			...
			else if(count($answerWords) != count($inputWords))
			{
				...
				if($whole_answer_length == $whole_input_length)
				{
					...
					if($whole_answer_length == $sim && $percent == 100)
					{
					...
					if($whole_answer_length - $sim ==  )
					
				}
				
				
				if($input_space_count != $answer_space_count)
				{
					if($input_space_count < $answer_space_count)
					{
						//missed a space
					}
					else if($input_space_count > $answer_space_count)
					{
						//extra space
					}
				}
